...
liegen_flussabwaerts(S1,S2):- S1\==S2, not(liegen_am_selben_fluss_flussabwaerts(S1,S2)),suchen(F,S1),
...
fahrt_abwaerts_ueber_denselben_fluss(S1,S2,[]):- not(liegen_am_selben_fluss_flussabwaerts(S1,S2)).
...
%Lösung 7
fahrt_aufwaerts(S1, S2, L) :- fahrt_abwaerts(S2, S1, L2), umkehren(L2, L).

%Lösung 8
fahrt(S1,S2,F):- liegen_flussabwaerts(S1,S2), fahrt_abwaerts(S1,S2,F).
fahrt(S1,S2,F):- liegen_flussaufwaerts(S1,S2), fahrt_abwaerts(S1,S2,F).
fahrt(S1,S2,F):- not(liegen_flussabwaerts_flussaufwaerts(S1,S2,S3), fahrt(S1, S3, F2), fahrt(S3, S2 , F3), vorletztes_element(F2,E1), zweites_element(F3, E2), E1 \== E2, zusammenfuegen(F1, F2, F).
